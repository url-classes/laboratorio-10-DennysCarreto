  ...
  class RBTree {
      ...
      private searchNode(nodoInicio: NodeRBT, referencia: number): void | string{
        if (nodoInicio == this.leaf)
            return '-1'
        else{
            if (nodoInicio.getData() == referencia)
                return 'El nodo '+referencia+' se encontro'
            else if (referencia < nodoInicio.getData())
                return this.searchNode(nodoInicio.getLeftChild(), referencia)
            else if (referencia > nodoInicio.getData())
                return this.searchNode(nodoInicio.getRightChild(), referencia)
        }
    }
      
      public search(referencia: number): string | void {
        return this.searchNode(this.root, referencia);
      }
